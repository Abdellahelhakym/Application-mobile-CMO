...
createOffer.post('/commande', (req, res) => {
    ...
    db.query('SELECT MAX(id) AS max_id FROM fiche_poste', (err, results) => {
        ...
        db.query(
            'INSERT INTO fiche_poste (id_societe, id_fiche_poste, intitule_poste, nom_responsable, contrat, duree, date_besoin, date_fin, lieu_travail, nbr_poste, salaire_proposer, logement, permis, commentaire, commentaire2 , statut_fiche, deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )',
            [
                payload.token_id,
                nextFicheId,
