...
const MessagerieCan = express.Router();
const auth = require('../middleware/auth');

MessagerieCan.get('/', auth, (req, res) => {
    res.send('Messagerie candidat route');
});

MessagerieCan.post('/getMessages', auth, (req, res) => {
...
MessagerieCan.post('/getSousMessages', auth, (req, res) => {
...
MessagerieCan.post('/CreateMessage', auth, (req, res) => {
  const token_id = req.user.token_id;
    ...
    db.query(
        ...
        VALUES (?, 'Candidat', ?, ?, CURDATE(), CURTIME(), 10, 10, 0)`,
        ...
        (err, result) => {
            ...
            db.query(
                ...
                VALUES ('Candidat', ?, ?, ?, 10, CURDATE(), CURTIME(), 10, 0)`,
...
MessagerieCan.post('/SendMessage', auth, (req, res) => {
...
MessagerieCan.post('/ClotureMessage', auth, (req, res) => {
...
MessagerieCan.post('/Notification', auth, (req, res) => {
    ...
    const sql = `
        ...
        WHERE deleted = 0
          ...
          AND id_msg IN (
              SELECT id
              FROM messagerie
              WHERE statut = 2 AND type_message = 'Candidat'
                AND deleted = 0
                AND id_user = ?
          )
    `;
